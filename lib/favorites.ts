...
//
//   Path B — name + source_ref match against single-food favorited
//            saved_meals' inner food keys. Used for never-favorited foods
//            (USDA-sourced, legacy null-source, etc.).
//
...
export function emptyFavorites(): Favorites {
  return { savedMealIds: new Set(), foodKeys: new Set() }
}
...
export function isFavoriteFood(food: FoodItem, favorites: Favorites): boolean {
  ...
  // Path B — name + source_ref dedup key
